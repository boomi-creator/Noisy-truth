...
class UKF:
    def __init__(self, num_states, process_noise, initial_state, initial_covar, alpha, k, beta, iterate_function):
        ...
        self.covar_weights[0] = (self.lambd/(self.n_dim + self.lambd))+(1-self.alpha**2+self.beta)
    ...
    def __get_sigmas(self):
        ...
        for i in range(self.n_dim):
            ret[i + 1] = self.x + sqrt_mat[:, i]
            ret[self.n_dim + i + 1] = self.x - sqrt_mat[:, i]
        return ret.T
